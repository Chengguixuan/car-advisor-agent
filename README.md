...
| 模块 | 用例 | 覆盖 |
|------|------|------|
| `test_config.py` | 21 | 默认值 / 环境变量 / 非法值降级 |
| `test_llm_client.py` | 22 | JSON 解析（3 策略）/ 异常翻译（401/429/网络） |
| `test_main.py` | 53 | 指令判断 / 历史裁剪 / 响应展示 / 空输入 |
...
| # | 输入 | 期望 | 结果 |
|---|------|------|------|
| 1 | "20万左右的SUV，家庭用，要省油" | >=1 款推荐 | ✅ 3 款 |
| 2 | "预算15万，纯电轿车，上下班用" | >=1 款推荐 | ✅ 2 款 |
| 3 | "帮我推荐一款车" | 有追问 | ✅ 追问不为空 |
| 4 | 空字符串 | 跳过 | ✅ 不发送请求 |
...
| # | 测试 | 结果 | 耗时 |
|---|------|------|------|
| 1 | `GET /health` | ✅ 200 | 0.3s |
| 2 | `POST /chat` — 20万混动SUV | ✅ 推荐 + 结构化输出 | 9.6s |
| 3 | 多轮对话（同 thread） | ✅ 上下文保持 | — |
| 4 | 线程隔离（不同 thread） | ✅ 互不干扰 | — |
| 5 | `POST /chat/stream` SSE | ✅ 8 events | — |

SSE 事件序列：`start → tool_call → tool_result → ... → final_recommendation → done`
